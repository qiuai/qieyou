...
define('page/item/index', function(){
	...
	$('#slider').on('click', 'li', function(e){
		e.preventDefault();
		pictureView({list: BASEPREVIEW, swipeTo: $(this).index()});
	}).swiper({
		pagination: '#btnBox'
	});
	$('#detail_content').on('click', '[node-type="pic"]', function(){
		pictureView({list: DETAILPREVIEW, swipeTo: $(this).index()});
	});
...
define('page/item/comment', function(){
	...
	$(document).on('click', '[node-type="pictureView"]', function(e){
		e.preventDefault();
		data.pictureView ? data.pictureView.elem.show() : (data.pictureView = pictureView({list: COMMENT_IMG}));
	});
