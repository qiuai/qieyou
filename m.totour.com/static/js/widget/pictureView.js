 ...
 */
var iSlider, islider_core, plugins_islider_animate, plugins_islider_zoom, plugins_islider_button, plugins_islider_dot, islider;
iSlider = function () {
  
  var iSlider = function (opts) {
    if (islider){
      islider.wrap.style.display = 'block';
      islider._opts.animateType = opts.animateType || 'default';
      islider._opts.data = opts.data;
      islider.reset();
      islider.slideTo(opts.initIndex || 0)
      return;
    }
    if (!(this instanceof iSlider)) {
      return new iSlider(opts);
    }
    if (!opts.dom) {
      opts.dom = document.createElement('div');
      opts.dom.className = 'pictureView';
  ...
  iSlider.prototype._setting = function () {
    ...
    this.useZoom = opts.useZoom || true;
  ...
  */
  iSlider.prototype._bindHandler = function () {
    ...
    outer.addEventListener('click', this);
...
plugins_islider_zoom = function (iSlider) {
  ...
  function handleDoubleTap(evt) {
    ...
    if (this.currentScale != 1) {
      touche = evt.touches[0];
      node.style.webkitTransformOrigin = generateTransformOrigin(touche.pageX - pos.left, touche.pageY - pos.top);
    }
  }
  //缩放图片
