	...
	<div class="content">
		...
		<table class="form table-form">
            ...
            <tr>
                ...
                <td><label><textarea class="w600 textEdit" rows="10" cols="103" name="note"><?php echo $product['note']?></textarea></label>
				</td>
            </tr>
			<tr>
                <td class="leftLabel"><cite>*</cite>有发票：</td>
                <td><label><textarea class="w600 textEdit" rows="10" cols="103" name="receipt"><?php echo $product['note']?></textarea></label>
				</td>
            </tr>
			<tr>
					...
					<div class="imageEdit roomImage">
						<ul class="clearfix wp50" id="D_imageView" style="float:left">
							<?php if(!empty($product['detail_images'])):?>
								...
								<?php foreach($images as $key => $image):?>
								<li><div class="preview"><img src="<?php echo $staticUrl.$image;?>" alt=""/></div>
									<input type="hidden" name="detail_images[]" value="<?php echo $image;?>"> <a href="javascript:void(0)" class="deleteImg">删除</a>
								</li>
								<?php endforeach;?>
								<?php $key += 1;?>
							<?php else:?>
			...
			<tr>
				...
				<td class="submitBtnList">
					...
					<a class="submit buttonG mr10" href="<?php echo $baseUrl.'product/tuanedit?pid='.$product['product_id'];?>">上架成团购</a>
					<?php endif;?>
            ...
            editor.loadPlugin('multiimage', function() {
                editor.plugin.multiImageDialog({
                    clickFn : function(urlList) {
                        ...
                        K.each(urlList, function(i, data) {
                            elem.append('<li><div class="preview"><img src="<?php echo $staticUrl;?>' + data.url + '" /></div><input type="hidden" name="detail_images[]" value="'+ data.url +'"> <a href="javascript:void(0)" class="deleteImg">删除</a></li>');
                            imageNum2 -= 1;
                            imgHasNum2 += 1;
                        });
