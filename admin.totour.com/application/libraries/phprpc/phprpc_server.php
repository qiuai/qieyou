...
class PHPRPC_Server {
    ...
    function sendFunctions() {
        $this->buffer .= "phprpc_functions=\"" . $this->encodeString(serialize(array_keys($this->functions))) . "\";\r\n";
        $this->sendCallback();
    }
    ...
    function callFunction() {
        ...
        if (array_key_exists($function, $this->functions)) {
            ...
            $result = $this->encodeString($this->encryptString(serialize($this->call($function, $arguments)), 2));
            $output = ob_get_clean();
            $this->buffer .= "phprpc_result=\"$result\";\r\n";
            if ($this->ref) {
                $arguments = $this->encodeString($this->encryptString(serialize($arguments), 1));
    ...
    function keyExchange() {
        ...
        if ($this->encrypt === true) {
            ...
            $this->buffer .= "phprpc_encrypt=\"" . $this->encodeString(serialize($encrypt)) . "\";\r\n";
            if ($this->keylen != 128) {
