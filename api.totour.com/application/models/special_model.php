...
class Special_model extends MY_Model {
	...
	public function get_category()
	{
		...
		if(!$cat&&$cat !== array())
		{
			...
			{
				$cat['title'][] = $row;
			//	$cat['list'][$row['id']][0] = array('category_id' => '0','name' => '全部');
			}
			$cond = array(
			...
			{
				$cat['list'][$row['category']][] = array('category_id' => $row['category_id'],'name' => $row['name']);
			}*/
	...
	public function get_local($city_id)
	{
		...
		if(!$local&&$local !== array())
		{
			...
			foreach($rs as $key => $row)
			{
				$local['title'][] = $row;
				$dests[] = $row['dest_id'];
			//	$local['list'][$row['dest_id']][0] = array('local_id' =>'0','local_name' => '全部');
			}
			$cond = array(
			...
			foreach($list as $key => $row)
			{
				$local['list'][$row['dest_id']][] = array('local_id' => $row['local_id'],'local_name' => $row['local_name']);
			}*/
