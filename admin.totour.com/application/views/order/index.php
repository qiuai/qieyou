...
<div class="filter mb20">
    <div class="form" style="padding:10px 36px;">
        ...
        <div class="fr">搜索：
			<select name="key" id="key" class="searchOrder">
    ...
    <div class="form" style="padding:10px 36px;">
        开始日期：<input type="text" onFocus="WdatePicker({onpicked:WdataOnPick('st'),doubleCalendar:true,maxDate:'%y-%M-%d'})" value="<?php echo $starttime?date('Y-m-d',$starttime):'';?>" name="st" title="请选择开始日期" id="startTime" class="Wdate mr40 ml5" readOnly>
        截止日期：<input type="text" onFocus="WdatePicker({onpicked:WdataOnPick('ed'),doubleCalendar:true,maxDate:'%y-%M-%d'})" value="<?php echo $endtime?date('Y-m-d',$endtime):'';?>" name="ed" title="请选择开始日期" id="endTime" class="Wdate mr40" readOnly>
...
<script type="text/javascript">
    $(function(){	
		...
		$('#keywordButton').click(function(){
			...
			{
				window.location.href="<?php echo $baseUrl;?>order/index?key="+key+"&keyword="+keyword;
			}
			else
			{
				alert('error');	
