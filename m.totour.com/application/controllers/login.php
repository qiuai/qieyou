...
class Login extends MY_Controller {
	...
	*/
    public function forgetPwd()
    {
		...
		$token = check_empty($this->input->post('token'),FALSE,'5013');
	...
	**/
	public function userregpost()
	{
		...
		if($user_id)
		{
			...
			$this->set_current_data($session);	//帮助用户登录
		}
		response_code('1');
	}
	...
	public function userLogin()
	{
		...
		$user_info = $this->model->get_user_detail($user['user_id']);		//待修正
	...
	*/
	...
	{
		$this->moduleTag = '找回密码';
		$token = md5(rand(0,999999).TIME_NOW);
		$this->set_current_data(array('token' => $token));
		$this->viewData = array(
			'token' => $token
		);
		$this->layout = 'simple';
